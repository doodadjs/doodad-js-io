	...
	global.DD_MODULES['Doodad.IO'] = {
		...
		create: function create(root, /*optional*/_options) {
								...
								ioInterfaces.Listener,
			{
				...
				// TODO: Pipe mechanism (instead of manually using a PipeStream)
				// TODO: Pipe to NodeJS streams directly
				//pipe: doodad.PUBLIC(doodad.MUST_OVERRIDE()), // function(/* <<< optional[transformFn], stream*/)
			})));
			...
			{
				$TYPE_NAME: 'PipeStream',
				
				__inputStream: doodad.PROTECTED(null),
				__outputStream: doodad.PROTECTED(null),

				__onInputStreamReady: doodad.PROTECTED(function onInputStreamReady(ev) {
					if (!ev.prevent) {
						this.__outputStream.write(ev.data.raw);
						ev.preventDefault();
					};
				}),
				
				create: doodad.OVERRIDE(function create(inputStream, outputStream, /*optional*/options) {
					this._super(options);

					if (root.DD_ASSERT) {
						root.DD_ASSERT(types._instanceof(inputStream, io.Stream), "Invalid input stream.");
						root.DD_ASSERT(types._instanceof(outputStream, io.OutputStream), "Invalid output stream.");
					};
					
					this.__inputStream = inputStream;
					this.__outputStream = outputStream;
					
					if (this.__inputStream instanceof io.InputStream) {
						this.__inputStream.onReady.attach(this, this.__onInputStreamReady);
					} else {
						this.__inputStream.onWrite.attach(this, this.__onInputStreamReady);
					};
				}),

				reset: doodad.OVERRIDE(function reset() {
					this.__inputStream.reset();
					this.__outputStream.reset();
				}),
				clear: doodad.OVERRIDE(function clear() {
					this.__inputStream.clear();
					this.__outputStream.clear();
				}),
			}));
		
			io.REGISTER(io.PipeStream.$extend(
			{
				$TYPE_NAME: 'TextPipeStream',
				
				__onInputStreamReady: doodad.REPLACE(function onInputStreamReady(ev) {
					if (!ev.prevent) {
						if (this.__outputStream instanceof io.ConsoleOutputStream) {
							this.__outputStream.write(ev.data.text);
						} else {
							this.__outputStream.writeText(ev.data.text);
						};
						ev.preventDefault();
					};
				}),
				...
				create: doodad.OVERRIDE(function create(inputStream, outputStream, /*optional*/options) {
					if (root.DD_ASSERT) {
						root.DD_ASSERT(types._implements(inputStream, [ioMixIns.TextInput, ioMixIns.TextOutput]), "Invalid input stream.");
						root.DD_ASSERT(types._implements(outputStream, ioMixIns.TextOutput) || (outputStream instanceof io.ConsoleOutputStream), "Invalid output stream.");
					};
					
					this._super(inputStream, outputStream, options);
				}),
			}));
